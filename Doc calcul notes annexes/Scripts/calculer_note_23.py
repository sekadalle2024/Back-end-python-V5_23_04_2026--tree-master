...
"""
Script de calcul de la NOTE 23 - AUTRES ACHATS
Syscohada Révisé

Ce script calcule la Note 23 à partir des balances N, N-1, N-2 en utilisant
...
de l'exercice pour les autres achats (fournitures, emballages, etc.).
...
class CalculateurNote23(CalculateurNote):
    """
    Calculateur pour la Note 23 - Autres Achats.
    
    Cette classe hérite de CalculateurNote et implémente le calcul spécifique
    de la Note 23 avec les autres achats (fournitures, emballages, etc.).
    
    Mapping des comptes SYSCOHADA:
    - Comptes 604, 605, 606, 607, 608: Autres achats
    ...
    - Autres achats (exercice N)
    - Autres achats (exercice N-1)
    - Variation N / N-1
    """
    
    def __init__(self, fichier_balance: str):
        """
        Initialise le calculateur de la Note 23.
        
        Args:
            fichier_balance: Chemin vers le fichier Excel des balances
        """
        super().__init__(fichier_balance, "23", "AUTRES ACHATS")
        
        # Mapping des comptes pour la Note 23
        # Note: Pour les comptes de charges, on utilise les comptes 604, 605, 606, 607, 608
        self.mapping_comptes = {
            'Autres achats': {
                'comptes': [
                    '604', '6041', '6042', '6043', '6044', '6045', '6046', '6047', '6048', '6049',
                    '605', '6051', '6052', '6053', '6054', '6055', '6056', '6057', '6058', '6059',
                    '606', '6061', '6062', '6063', '6064', '6065', '6066', '6067', '6068', '6069',
                    '607', '6071', '6072', '6073', '6074', '6075', '6076', '6077', '6078', '6079',
                    '608', '6081', '6082', '6083', '6084', '6085', '6086', '6087', '6088', '6089'
                ]
            }
        }
    ...
    def generer_note(self) -> pd.DataFrame:
        """
        Génère la Note 23 complète avec les autres achats N et N-1.
        
        Cette méthode:
        1. Calcule les autres achats pour l'exercice N
        2. Calcule les autres achats pour l'exercice N-1
    ...
    def generer_html(self, df: pd.DataFrame) -> str:
        ...
        colonnes_config = {
            'groupes': [
                {'titre': 'AUTRES ACHATS', 'colonnes': ['montant', 'mouvements_debit', 'mouvements_credit']}
            ],
            'en_tetes': {
                'libelle': 'EXERCICE',
                'montant': 'Montant',
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Calcul de la Note 23 - Autres Achats'
    )
    parser.add_argument(
        'fichier_balance',
        nargs='?',
    ...
    parser.add_argument(
        '--output-html',
        default='../Tests/test_note_23.html',
        help='Chemin du fichier HTML de sortie (défaut: ../Tests/test_note_23.html)'
    )
    parser.add_argument(
        '--output-trace',
        default='../Tests/note_23_trace.json',
        help='Chemin du fichier de trace JSON (défaut: ../Tests/note_23_trace.json)'
    )
    
    args = parser.parse_args()
    
    # Créer le calculateur
    calculateur = CalculateurNote23(args.fichier_balance)
