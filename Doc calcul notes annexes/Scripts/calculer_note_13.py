"""
Calculateur Note 13 - Dettes Fournisseurs

Ce module calcule la Note 13 des annexes SYSCOHADA Révisé concernant les dettes
fournisseurs. Il traite les comptes de la classe 40X.
...
Structure de la note:
    - Solde d'ouverture
    - Augmentations (nouveaux achats)
    - Règlements
    - Solde de clôture

Usage:
    python calculer_note_13.py
"""
...
class CalculateurNote13(CalculateurNote):
    """Calculateur pour la Note 13 - Dettes Fournisseurs."""
    
    def __init__(self, fichier_balance: str):
        super().__init__(fichier_balance, "13", "Dettes Fournisseurs")
        
        self.mapping_comptes = {
            "Fournisseurs - dettes en compte": ["401"],
            "Fournisseurs - effets à payer": ["402"],
            "Fournisseurs - retenues de garantie": ["403"],
    ...
    def calculer_ligne_dette(self, libelle: str, comptes: list) -> Dict[str, float]:
        """Calcule une ligne de dette fournisseur."""
    ...
    def generer_note(self) -> pd.DataFrame:
        """Génère la Note 13 complète."""
        lignes = []
        
        print(f"  Calcul des dettes fournisseurs...")
        
        for libelle, comptes in self.mapping_comptes.items():
            ligne = self.calculer_ligne_dette(libelle, comptes)
            lignes.append(ligne)
        ...
        total = {
            'libelle': 'TOTAL DETTES FOURNISSEURS',
    ...
    def generer_html(self, df: pd.DataFrame) -> str:
        """Génère le fichier HTML de la Note 13."""
        from html_generator import HTMLGenerator
        
        colonnes_config = {
            'groupes': [
                {'titre': 'MOUVEMENTS DES DETTES FOURNISSEURS', 'colonnes': ['solde_ouverture', 'augmentations', 'reglements', 'solde_cloture']}
            ],
            'en_tetes': {
...
def main():
    ...
    fichier_html = tests_dir / "test_note_13.html"
    fichier_trace = tests_dir / "trace_note_13.json"
    
    calculateur = CalculateurNote13(str(fichier_balance))
