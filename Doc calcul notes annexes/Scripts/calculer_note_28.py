...
"""
Script de calcul de la NOTE 28 - VENTES DE MARCHANDISES
Syscohada Révisé

Ce script calcule la Note 28 à partir des balances N, N-1, N-2 en utilisant
...
de l'exercice pour les ventes de marchandises.
...
class CalculateurNote28(CalculateurNote):
    """
    Calculateur pour la Note 28 - Ventes de Marchandises.
    
    Cette classe hérite de CalculateurNote et implémente le calcul spécifique
    de la Note 28 avec les ventes de marchandises.
    
    Mapping des comptes SYSCOHADA:
    - Comptes 70X: Ventes de marchandises
      * 701: Ventes de marchandises
      * 7011-7019: Sous-comptes de ventes de marchandises
    
    Structure de la note:
    - Ventes de marchandises (exercice N)
    - Ventes de marchandises (exercice N-1)
    - Variation N / N-1
    """
    
    def __init__(self, fichier_balance: str):
        """
        Initialise le calculateur de la Note 28.
        
        Args:
            fichier_balance: Chemin vers le fichier Excel des balances
        """
        super().__init__(fichier_balance, "28", "VENTES DE MARCHANDISES")
        
        # Mapping des comptes pour la Note 28
        # Note: Pour les comptes de produits, on utilise uniquement les comptes 701
    ...
    def generer_note(self) -> pd.DataFrame:
        """
        Génère la Note 28 complète avec les ventes N et N-1.
        
        Cette méthode:
        1. Calcule les ventes de marchandises pour l'exercice N
        2. Calcule les ventes de marchandises pour l'exercice N-1
    ...
    def generer_html(self, df: pd.DataFrame) -> str:
        ...
        colonnes_config = {
            'groupes': [
                {'titre': 'VENTES DE MARCHANDISES', 'colonnes': ['montant', 'mouvements_credit', 'mouvements_debit']}
            ],
            'en_tetes': {
                'libelle': 'EXERCICE',
                'montant': 'Montant',
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Calcul de la Note 28 - Ventes de Marchandises'
    )
    parser.add_argument(
        'fichier_balance',
        nargs='?',
    ...
    parser.add_argument(
        '--output-html',
        default='../Tests/test_note_28.html',
        help='Chemin du fichier HTML de sortie (défaut: ../Tests/test_note_28.html)'
    )
    parser.add_argument(
        '--output-trace',
        default='../Tests/note_28_trace.json',
        help='Chemin du fichier de trace JSON (défaut: ../Tests/note_28_trace.json)'
    )
    
    args = parser.parse_args()
    
    # Créer le calculateur
    calculateur = CalculateurNote28(args.fichier_balance)
