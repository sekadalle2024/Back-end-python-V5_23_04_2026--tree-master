"""
Calculateur Note 20 - Écarts de Conversion Passif

Ce module calcule la Note 20 des annexes SYSCOHADA Révisé concernant les écarts de conversion passif.
Il traite le compte 478 (Écarts de conversion - Passif).
...
class CalculateurNote20(CalculateurNote):
    """Calculateur pour la Note 20 - Écarts de Conversion Passif."""
    
    def __init__(self, fichier_balance: str):
        super().__init__(fichier_balance, "20", "Écarts de Conversion Passif")
        
        self.mapping_comptes = {
            "Écarts de conversion - Passif": ["478"]
        }
    
    def calculer_ligne_ecarts_conversion(self, libelle: str, comptes: list) -> Dict[str, float]:
        """Calcule une ligne d'écarts de conversion passif."""
        ...
        # Pour les écarts de conversion passif (compte de passif - gains latents), 
        # on utilise les soldes créditeurs
    ...
    def generer_note(self) -> pd.DataFrame:
        """Génère la Note 20 complète."""
        lignes = []
        
        print(f"  Calcul des écarts de conversion passif...")
        
        for libelle, comptes in self.mapping_comptes.items():
            ligne = self.calculer_ligne_ecarts_conversion(libelle, comptes)
            lignes.append(ligne)
    ...
    def generer_html(self, df: pd.DataFrame) -> str:
        """Génère le fichier HTML de la Note 20."""
        from html_generator import HTMLGenerator
        
        colonnes_config = {
            'groupes': [
                {'titre': 'MOUVEMENTS DES ÉCARTS DE CONVERSION PASSIF', 'colonnes': ['solde_ouverture', 'augmentations', 'diminutions', 'solde_cloture']}
            ],
            'en_tetes': {
                'libelle': 'NATURE',
...
def main():
    ...
    fichier_html = tests_dir / "test_note_20.html"
    fichier_trace = tests_dir / "trace_note_20.json"
    
    calculateur = CalculateurNote20(str(fichier_balance))
