...
"""
Script de calcul de la NOTE 22 - ACHATS DE MATIÈRES PREMIÈRES
Syscohada Révisé

Ce script calcule la Note 22 à partir des balances N, N-1, N-2 en utilisant
...
de l'exercice pour les achats de matières premières et fournitures liées.
...
class CalculateurNote22(CalculateurNote):
    """
    Calculateur pour la Note 22 - Achats de Matières Premières.
    
    Cette classe hérite de CalculateurNote et implémente le calcul spécifique
    de la Note 22 avec les achats de matières premières et fournitures liées.
    
    Mapping des comptes SYSCOHADA:
    - Comptes 601, 602: Achats de matières premières
      * 601: Achats de marchandises (matières premières)
      * 602: Achats de matières premières et fournitures liées
    
    Structure de la note:
    - Achats de matières premières (exercice N)
    - Achats de matières premières (exercice N-1)
    - Variation N / N-1
    """
    
    def __init__(self, fichier_balance: str):
        """
        Initialise le calculateur de la Note 22.
        
        Args:
            fichier_balance: Chemin vers le fichier Excel des balances
        """
        super().__init__(fichier_balance, "22", "ACHATS DE MATIÈRES PREMIÈRES")
        
        # Mapping des comptes pour la Note 22
        # Note: Pour les comptes de charges, on utilise les comptes 601 et 602
        self.mapping_comptes = {
            'Achats de matières premières': {
                'comptes': ['601', '6011', '6012', '6013', '6014', '6015', '6016', '6017', '6018', '6019',
                           '602', '6021', '6022', '6023', '6024', '6025', '6026', '6027', '6028', '6029']
            }
        }
    ...
    def generer_note(self) -> pd.DataFrame:
        """
        Génère la Note 22 complète avec les achats N et N-1.
        
        Cette méthode:
        1. Calcule les achats de matières premières pour l'exercice N
        2. Calcule les achats de matières premières pour l'exercice N-1
    ...
    def generer_html(self, df: pd.DataFrame) -> str:
        ...
        colonnes_config = {
            'groupes': [
                {'titre': 'ACHATS DE MATIÈRES PREMIÈRES', 'colonnes': ['montant', 'mouvements_debit', 'mouvements_credit']}
            ],
            'en_tetes': {
                'libelle': 'EXERCICE',
                'montant': 'Montant',
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Calcul de la Note 22 - Achats de Matières Premières'
    )
    parser.add_argument(
        'fichier_balance',
        nargs='?',
    ...
    parser.add_argument(
        '--output-html',
        default='../Tests/test_note_22.html',
        help='Chemin du fichier HTML de sortie (défaut: ../Tests/test_note_22.html)'
    )
    parser.add_argument(
        '--output-trace',
        default='../Tests/note_22_trace.json',
        help='Chemin du fichier de trace JSON (défaut: ../Tests/note_22_trace.json)'
    )
    
    args = parser.parse_args()
    
    # Créer le calculateur
    calculateur = CalculateurNote22(args.fichier_balance)
